#!/bin/bash
# Condition A (trained tracker), seed=2
set -e
...
echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] START: Condition A seed=2"
python3 train.py \
    ...
    use_wandb=false \
    exp_name=condA_s2 \
    2>&1 | tee "$OUTDIR/condA_s2.log"

echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] DONE: Condition A seed=2"
