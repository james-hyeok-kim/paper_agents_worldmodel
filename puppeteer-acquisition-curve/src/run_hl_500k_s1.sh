#!/bin/bash
# High-level corridor training with tracker@500k, seed=1 — GPU 1
set -e
...
LOGFILE="/home/jovyan/workspace/paper_agents_worldmodel/experiments/wip/puppeteer-acquisition-curve/ablation_500000_s1.log"

# Wait for checkpoint if not yet available (max 4 hours)
MAX_WAIT=14400
WAITED=0
...
cd "$PUPPETEER_DIR"
echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] START: tracker@500k seed=1 GPU=1" | tee "$LOGFILE"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] DONE: tracker@500k seed=1" | tee -a "$LOGFILE"
