#!/bin/bash
# Condition B (random tracker), seed=1
set -e
...
echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] START: Condition B seed=1"
python3 train.py \
    ...
    use_wandb=false \
    exp_name=condB_s1 \
    2>&1 | tee "$OUTDIR/condB_s1.log"

echo "[$(date '+%Y-%m-%d %H:%M:%S KST')] DONE: Condition B seed=1"
